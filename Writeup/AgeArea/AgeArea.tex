...
Define a function for a migratory history $D_{H_{k}}=m(n^*_{H_k}$,N(H_k)).$ that is strictly increasing in the first argument and strictly decreasing in the second. Define the \textbf{Dyen Divergence} of culture $k$ in terms of the history $H^*_k$ as:
\begin{equation*}
D_k=\max \{D_{H_{1k}},D_{H_{2k}},\hdots D_{H_{Ik}} \}
\end{equation*}
\end{definition}

The above definition defines not `a' measure of divergence, but a family of such measures. The measure is increasing in the number of events associated with the largest chain in the history, and decreasing in the number of chains in the history. I\ take the largest such value associated with each location as the Dyen Divergence of culture $k$. Using the probability functions and this measure of divergence, one can now show that larger divergences coincide with more likely chains.  


\subsection{ Age-Area Theorem}

I am now in a position to state and prove the following theorem.

\begin{theorem}[Age-Area Theorem]
Suppose that Assumptions 1 hold, and define a Dyen Divergence as in definition 1. Then  
\begin{equation*}
...
\begin{equation*}
dp=p\left(1-\frac{p}{K}\right)+\sigma p dz
\end{equation*}

This exact parameterization in terms of a stochastic logistic growth model is not the critical fact of the matter. What is crucial is the mean-reverting nature of the stochastic differential equation (\ref{sde}). \cite{rdgn99} show that the above process has a time-independent, initial-condition-independent stationary distribution given by:
\begin{equation*}
W(p) = \left( \Gamma\left[\frac{2r}{\sigma^2}-1\right]\right)^{-1}\left(\frac{2r}{K\sigma^2}\right)^{\frac{2r}{\sigma^2}-1}p^{\frac{2r}{\sigma^2}-2}\exp\left[\frac{2rp}{K\sigma^2}\right]
\end{equation*}

As \cite{rdgn99} and \cite{nrs85} show, the existence of a time-independent steady state-density implies that the first passage time to a `large' barrier is approximately exponential. That is, let $g(B,t|p_0)$ denote the distribution of first passage times to a barrier $B$ given initial population $p_0$ . Then:
\begin{equation*}
g(B,t|p_o) \sim \frac{1}{t_1(B|p_0)}\exp\left(-\frac{t}{t_1(B|p_0)}}\right) 
\end{equation*}

Where $t_1(B,p_0)$ is the mean first passage time corresponding with the disribution.\footnote{\cite{rdgn99} give formulas for calculating moments.} That is, the distribution of times to which the barrier is attained is approximately exponential. 

Before fleshing out the details, it is helpful to develop a visual picture of how this process works. Figure \ref{evo0} shows a process moving along through time at a high carrying capacity. At some point, the carrying capacity is exhausted, and a large segement of the population leaves. The new carrying capacity in the location is evidently lower.
